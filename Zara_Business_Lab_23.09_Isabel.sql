...
SELECT*FROM clientes WHERE nombre_cliente LIKE '%Andres%';

SELECT*FROM compras WHERE id_cliente = 2;

DELETE FROM compras WHERE id_compra IN (SELECT id_compra FROM (SELECT id_compra FROM compras WHERE monto_total < 50.00) AS t);
...
SELECT*FROM empleados WHERE fecha_contratacion> 22-01-01;
